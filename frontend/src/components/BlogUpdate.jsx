...
const BlogUpdate = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:8080/api/v1/update/${blogId}`, blogDetails);
            toast.success("Blog updated successfully!");
            navigate(`/blogs/${blogId}`); // Redirect to the updated blog detail page
        } catch (error) {
    ...
    return (
        <div className="container mx-auto my-8 p-8 bg-white rounded-lg shadow-lg max-w-3xl">
            <h1 className="text-4xl font-bold text-gray-800 mb-4 text-center">Update Blog</h1>
