...
const BlogDetail = () => {
    ...
    useEffect(() => {
        const fetchBlogDetails = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/api/v1/fetchOne/${blogId}`);
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(`http://localhost:8080/api/v1/delete/${blogId}`);
    ...
    return (
        <div className="container mx-auto my-8 p-8 bg-white rounded-lg shadow-lg max-w-3xl">
            <h1 className="text-4xl font-bold text-gray-800 mb-4 text-center">Blog Details</h1>
            <div className="mb-6">
                <h2 className="text-2xl font-semibold text-blue-700 mb-2">Title: {blogDetails.title}</h2>
                <p className="text-gray-700 text-lg mb-4">Description: {blogDetails.description}</p>
